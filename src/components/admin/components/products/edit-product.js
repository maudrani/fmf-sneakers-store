...
import { UpdateProduct } from "../../../store/db/products";
...
const ProductForm = ({ product, onCancel, onSave }) => {
  ...
  const [images, setImages] = useState(product.images.x25);
  ...
  const onSubmit = (data) => {
    ...
    parsedData.price_original_quality = parseInt(data.price_original_quality);
    parsedData.category = category;
    parsedData._id = product._id;

    parsedData.images = { x50: images, x25: images, x15: images };

    editProduct(parsedData);
  };
  ...
  return (
    <PanelContainer w-100 white direction="c" style={{ minHeight: "80vh" }}>
      ...
      >
        ...
        <Container w-100 style={{margin: '0 0 5rem 0'}}>
          <Container w-100 direction="c" pw="sm">
            <Text ph="xs" dark-gray>
              Precio (Calidad AAA)
            </Text>
          ...
          <Container w-100 direction="c" pw="sm">
            <Text ph="xs" dark-gray>
            Precio (Calidad Original)
            </Text>
            <Input
              sm
              typeof="tel"
              name="price_original_quality"
              defaultValue={product.price_original_quality || 19000}
              style={{ padding: ".5rem 0", textAlign: "center" }}
              ref={register({
                required: { value: true, message: "Campo requerido " },
                pattern: {
            ...
            />
            <Text w-100 weight="light" red>
              {errors.price_original_quality?.message}
            </Text>
          </Container>
        </Container>
