...
const BascketIcon = ({ color = "whitesmoke", hideData = false, size }) => {
  const [cart] = useContext(CartContext);

  /* color = cart.length !== 0 ? 'yellow' : color; */
  ...
  return (
    <Container>
      {hideInfo() && (
        ...
        >
          <Text weight='black' style={{ fontSize: "0.6rem" }}>{lengthToShow()}</Text>
        </QuantityGlobe>
      )}
      <svg
        viewBox="0 -31 512.00026 512"
        style={{ width: size + "rem", minWidth: "1.3rem", fill: colors[color] }}
      >
